...
Oscillator::Oscillator(oscType initType, unsigned char initNote, unsigned int initFreq, unsigned int initDC, float initAmplitudeMax, unsigned int initAttack, unsigned int initDecay, float initSustainLevel, unsigned int initRelease) {
    this->toDelete = false;
    
    if (initType < sine || initType > square) this->type = sine; // max. is 3 for now, will be 4 later
    ...
    else this->frequency = initFreq;

    this->dutyCycle = initDC;
    if (this->dutyCycle < 1) this->dutyCycle = 1;
    else if (this->dutyCycle > 2046) this->dutyCycle = 2046;
    
    if (initAmplitudeMax <= 0.0) this->toDelete = true;
    else if (initAmplitudeMax > 1.0) this->amplitudeMax = 1;
