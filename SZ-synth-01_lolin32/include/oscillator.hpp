...
class Oscillator { // TODO: square waveform with separate behavior than other waveforms (no LUT, duty cycle)
public:
    oscType type;
    unsigned char note;
    unsigned int frequency; // the frequency the waveform is played at
    ...
    Oscillator(oscType initType, unsigned char initNote, unsigned int initFreq, unsigned int initDC, float initAmplitudeMax, unsigned int initAttack, unsigned int initDecay, float initSustainLevel, unsigned int initRelease);
    ~Oscillator();
